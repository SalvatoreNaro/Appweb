...
frase_label2 = customtkinter.CTkLabel(scrollable_frame, text="Programmatore di Ai e designer" ,text_color="white", font=("Arial", 12))
...
frameSalvo = customtkinter.CTkFrame(scrollable_frame,width=250,height=250,bg_color="Black",fg_color="Black")
frameSalvo.grid(row=31,column=0,padx=50,pady=50)


def politica():
    messagebox.showinfo(title="politica",message="Rispettiamo la politica")
def privacy():
    messagebox.showinfo(title="privacy",message="Rispettiamo la privacy")
def informazioni():
    messagebox.showinfo(title="informazioni",message="Le informazioni le trovate sul sito")

pulsantepolitica =  customtkinter.CTkButton(master = frameSalvo,fg_color="Black",bg_color="Black",text_color="white",text="POLITICA",font=("Arial",15),command = politica)
pulsantepolitica.grid(row=32,column=0,padx=20,pady=5)

pulsanteprivacy =  customtkinter.CTkButton(master = frameSalvo,fg_color="Black",bg_color="Black",text_color="white",text="PRIVACY",font=("Arial",15),command = privacy)
pulsanteprivacy.grid(row=32,column=1,padx=20,pady=5)

pulsanteinformazioni2 = customtkinter.CTkButton(master = frameSalvo, fg_color="black", bg_color="black",text_color="white",text="INFORMAZIONI",font=("Arial",15),command=informazioni)
pulsanteinformazioni2.grid(row=32,column=2,padx=10,pady=5)



fine = customtkinter.CTkFrame(scrollable_frame,width=250,height=250,bg_color="black",fg_color="black")
fine.grid(row=33,column = 0,padx=50,pady=50)

parolona= "Do you want login in my app?"
login_py = customtkinter.CTkLabel(master=fine,
                                  text=parolona,
                                  text_color="blue",
                                  fg_color="black",
                                  bg_color="black",
...
login_py.grid(row=34,column=1,padx=20,pady=5)
def switch_app():
    subprocess.Popen(["python","c:\\Users\\salva\\Downloads\\Exercises.py"]) #This file can be found in the login.py repository
    
button_login = customtkinter.CTkButton(master=fine,
                                       text="Login",
                                       text_color="white",
                                       fg_color="black",
                                       bg_color="black",
                                       font=("Arial",16),
                                       command= switch_app)
button_login.grid(row=35,column=2,padx=10,pady=10)

#rovescio = True
#testone=len(parolona)
#def animazione():
    #global rovescio,testone
    #if rovescio:
        #testone+=1
        #if testone > len(parolona):
            #rovescio = False
    #else:
        #testone-=1
        #if testone == 0:
            #rovescio=True
    #login_py.configure(text = parolona[:testone])
    #window.after(50, animazione)
#animazione()

riservatezza_frame = customtkinter.CTkFrame(master=scrollable_frame,
                                            fg_color="black",
                                            bg_color="black")
riservatezza_frame.grid(row=35,column=0,padx=50,pady=50)
riservatezza  = customtkinter.CTkLabel(master=riservatezza_frame,text="©Salvatore Naro. All rights reserved. Privacy Policy",text_color="White",fg_color="Black",bg_color="Black")
riservatezza.grid(row=36,column=1,padx=20,pady=15,sticky="we")
